...
def get_user_input():
    instrument = input("Choose an instrument (piano, trumpet, flute): ").lower()
    notes_and_durations_str = input("Enter the notes and durations to play (e.g., C4 1 D4 1 E4 2): ")
    
    # Split input into note names and durations
    ...
    # Create tuples of note names and durations
    notes_and_durations = list(zip(input_list[::2], input_list[1::2]))
    
    return instrument, notes_and_durations
...
def convert_and_play_wav():
    ...
    subprocess.run(['timidity', '-Ow', '-o', 'output.wav', 'output.mid'])
...
def main():
    instrument, notes_and_durations = get_user_input()
