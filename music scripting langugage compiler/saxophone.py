...
def get_user_input():
    notes_and_durations_str = input("Enter the notes and durations for saxophone (e.g., C4 1 D4 1 E4 2): ")
...
def create_midi_file(notes_and_durations):
    ...
    # Set the instrument to Saxophone (instrument number 65)
    ...
    # Save to a MIDI file
    mid.save('output_saxophone.mid')

def convert_and_play_wav():
    # Convert the MIDI file to WAV using Timidity
    subprocess.run(['timidity', '-Ow', '-o', 'out_saxophone.wav', 'output_saxophone.mid'])

    # Play the WAV file in Colab
    audio = Audio('out_saxophone.wav')
    display(audio)
