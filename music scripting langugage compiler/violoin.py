...
def get_user_input():
    notes_and_durations_str = input("Enter the notes and durations for violin (e.g., C4 1 D4 1 E4 2): ")
...
def create_midi_file(notes_and_durations):
    ...
    # Set the instrument to Violin (instrument number 41)
    ...
    # Save to a MIDI file
    mid.save('output_violin.mid')

def convert_and_play_wav():
    # Convert the MIDI file to WAV using Timidity
    subprocess.run(['timidity', '-Ow', '-o', 'out_violin.wav', 'output_violin.mid'])

    # Play the WAV file in Colab
    audio = Audio('out_violin.wav')
    display(audio)
