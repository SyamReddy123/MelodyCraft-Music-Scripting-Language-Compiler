...
def get_user_input():
    notes_and_durations_str = input("Enter the notes and durations for guitar (e.g., C4 1 D4 1 E4 2): ")
...
def create_midi_file(notes_and_durations):
    ...
    # Set the instrument to Acoustic Guitar (instrument number 24)
    ...
    # Save to a MIDI file
    mid.save('output_guitar.mid')

def convert_and_play_wav():
    # Convert the MIDI file to WAV using Timidity
    subprocess.run(['timidity', '-Ow', '-o', 'out_guitar.wav', 'output_guitar.mid'])

    # Play the WAV file in Colab
    audio = Audio('out_guitar.wav')
    display(audio)
