...
def get_user_input():
    notes_and_durations_str = input("Enter the notes and durations for flute (e.g., C4 1 D4 1 E4 2): ")
...
def create_midi_file(notes_and_durations):
    ...
    # Set the instrument to Flute (instrument number 73)
    ...
    # Save to a MIDI file
    mid.save('output_flute.mid')

def convert_and_play_wav():
    # Convert the MIDI file to WAV using Timidity
    subprocess.run(['timidity', '-Ow', '-o', 'out_flute.wav', 'output_flute.mid'])

    # Play the WAV file in Colab
    audio = Audio('out_flute.wav')
    display(audio)
