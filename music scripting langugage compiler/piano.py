...
def get_user_input():
    notes_and_durations_str = input("Enter the notes and durations for piano (e.g., C4 1 D4 1 E4 2): ")
...
def create_midi_file(notes_and_durations):
    ...
    # Set the instrument to Piano (instrument number 0)
    ...
    # Save to a MIDI file
    mid.save('output_piano.mid')

def convert_and_play_wav():
    # Convert the MIDI file to WAV using Timidity
    subprocess.run(['timidity', '-Ow', '-o', 'out_piano.wav', 'output_piano.mid'])

    # Play the WAV file in Colab
    audio = Audio('out_piano.wav')
    display(audio)
